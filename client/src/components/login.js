...
export const LoginComponent = (props) => {
  ...
  const submitLoginDetail = async () => {
    ...
    const loginUrl = `/auth/login/basic`
    storageActions.clearStorage()
    restActions.POST(loginUrl, data, config).then((res) => {
      if (res.data) {
        ...
        storageActions.storeItems(
          ['isAuthenticated', 'username',  'email', 'token'],
          [
            true,
            payload['name'],
            payload['email'],
      ...
      (err) => {
        setFormLoader(false)
        console.log('Login kar lo', err)
        if(err?.status === 401){
          NotificationMessage.showError('Invalid Login')
        }else {
        ...
        <div className='body-form'>
          <form>
            ...
            <div className='input-group mb-3'>
              ...
              <input type='password' required name="password" value={data?.password} id="password" className='form-control' placeholder='Password' onChange={inputsHandler} />
            </div>
          </form>
